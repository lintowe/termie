...
### Platform
- **termie runs on Linux.** The same rendering, terminal emulation, and plugin core now build and run on X11 and Wayland, alongside Windows. Config and data live under `$XDG_CONFIG_HOME/termie` (falling back to `~/.config/termie`); the shell picker offers `bash`, `zsh`, and `fish`, each launched with an integration hook that sources your own rc first and then adds the OSC 133 prompt marks and OSC 7 cwd reporting that drive prompt-jump and tab labels. Clipboard goes through the Wayland or X11 selection, links open with `xdg-open`, the bell raises the window-manager urgency hint, and the plugin sandbox is a bubblewrap jail instead of an AppContainer. The Windows-only integrations — default-terminal handoff, the taskbar jump list and progress, admin windows, the Quake global hotkey — compile away to nothing on Linux. A CI job builds, tests, and lints the Linux target on every push, and unit plus golden-render tests pass on both.
